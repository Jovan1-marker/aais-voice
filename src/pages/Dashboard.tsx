...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "aais_messages" }, () => reload())
      .subscribe();
  ...
  const filtered = (statuses: Message["status"][]) => {
    ...
    if (categoryFilter !== "all") msgs = msgs.filter((m) => m.type === categoryFilter);
    return msgs;
  };
  ...
  const statusBadge = (status: Message["status"]) => {
    switch (status) {
      ...
      case "solved": return <Badge className="bg-primary text-primary-foreground">Solved</Badge>;
    ...
    <Card className="border border-primary/30 shadow-sm animate-fade-in">
      <CardContent className="p-5">
        <div className="flex flex-wrap items-center gap-2 mb-2">
          ...
          <span className="ml-auto"><TimeAgo timestamp={msg.timestamp} /></span>
        </div>
  ...
  const HistorySection = ({ title, items, show, onToggle }: { title: string; items: Message[]; show: boolean; onToggle: () => void }) => (
    <section className="mb-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-primary">{title}</h3>
        <Button variant="ghost" size="sm" onClick={onToggle} className="text-muted-foreground">
          {show ? <>Hide <ChevronUp className="ml-1 h-4 w-4" /></> : <>+{items.length} in history <ChevronDown className="ml-1 h-4 w-4" /></>}
        </Button>
      </div>
      {show && (
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-3xl">
        ...
        <section className="mb-10">
          ...
          {pending.length === 0 ? (
            ...
            <div className="space-y-4">
              {visiblePending.map((msg) => <PendingCard key={msg.id} msg={msg} />)}
        ...
        <HistorySection title="Approved / Rejected History" items={approveRejectHistory} show={showHistory} onToggle={() => setShowHistory(!showHistory)} />
        <HistorySection title="Solved / Unsolved History" items={solveHistory} show={showSolveHistory} onToggle={() => setShowSolveHistory(!showSolveHistory)} />
      </main>
      <Footer />

      {lightbox && (
        <ImageLightbox src={lightbox.src} alt={lightbox.alt} open={!!lightbox} onOpenChange={(open) => !open && setLightbox(null)} />
      )}
    </div>
  );
};
